...
# BOT_TOKEN, CHANNEL, SUBSCRIBE_TO, SUGGEST_TO, ALLOWED_ADMINS, AUTOSIGN, ALBUM_WAIT_SEC
BOT_TOKEN = (os.getenv("BOT_TOKEN") or "").strip()

# IMPORTANT: CHANNEL must be '@username' (no t.me link). Example: @minskiyes
...
# comma-separated numeric IDs (e.g. "123,456"). If empty -> allow everyone.
ALLOWED_ADMINS_RAW = (os.getenv("ALLOWED_ADMINS") or "").strip()

# appended to the end of message (optional), e.g. "\n\n— @minskyes"
AUTOSIGN = (os.getenv("AUTOSIGN") or "").strip()

# collect media_group items window (seconds)
...
class Draft:
    ...
    medias: List[Union[InputMediaPhoto, InputMediaVideo, InputMediaDocument, InputMediaAudio]] = field(default_factory=list)
...
def _bold_title(text: str) -> str:
    """Makes the first line bold (HTML), treating it as a title."""
    if not text:
        return text
...
def _rebuild_media(m, caption_html: Optional[str], with_caption: bool):
    cap = caption_html if with_caption else None
    pm = ParseMode.HTML if with_caption and caption_html else None

    if isinstance(m, InputMediaPhoto):
        return InputMediaPhoto(media=m.media, caption=cap, parse_mode=pm)
    if isinstance(m, InputMediaVideo):
        return InputMediaVideo(media=m.media, caption=cap, parse_mode=pm)
    if isinstance(m, InputMediaDocument):
        return InputMediaDocument(media=m.media, caption=cap, parse_mode=pm)
    if isinstance(m, InputMediaAudio):
        return InputMediaAudio(media=m.media, caption=cap, parse_mode=pm)
    return m
...
async def _send_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, draft: Draft) -> str:
    ...
    if caption:
        caption = _bold_title(_safe_html(caption))

    # Album preview
    if draft.medias:
        medias = [_rebuild_media(m, caption, with_caption=(i == 0 and bool(caption))) for i, m in enumerate(draft.medias)]
...
async def _publish_draft(context: ContextTypes.DEFAULT_TYPE, draft_id: str) -> None:
    ...
    try:
        if draft.medias:
            ...
            # albums can't have inline keyboard -> send separate message
...
async def handle_album_item(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def finalize():
        ...
        medias: List[Union[InputMediaPhoto, InputMediaVideo, InputMediaDocument, InputMediaAudio]] = []
        for u in items:
            m = u.message
            if m.photo:
